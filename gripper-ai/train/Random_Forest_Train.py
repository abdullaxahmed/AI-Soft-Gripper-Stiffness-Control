...
class GripperAIController:
    ...
    def extract_features(self, time_series_data):
        ...
        # Calculate bending rate from recent points
        if len(bendings) >= 5:
        ...
        # Stability based on recent variance
        if len(bendings) >= 5:
            stability_factor = 1.0 / (1.0 + np.var(bendings[-5:]))
        else:
            stability_factor = 0.5

        # How quickly we reached current bending
    ...
    def prepare_training_data(self, csv_file_path):
        ...
        for trial_num in df['trial_number'].unique():
            ...
            for _, row in trial_data.iterrows():
                time_series.append([row['time_seconds'], row['pump_speed_pwm'], row['bending_value']])
            
            # Only use trials with sufficient data
        ...
        # Sample points throughout each trial for training
        for trial in trials:
            ...
            for point_idx in sample_points:
                if point_idx < len(time_series):
                    ...
                    # Target: should stop soon (binary based on progress)
    ...
    def predict_stop_probability(self, current_time_series):
        ...
        stop_probability = self.model.predict(features_scaled)[0]
...
if __name__ == "__main__":
    ...
    try:
        mse, r2 = controller.train_model(CSV_FILE)
